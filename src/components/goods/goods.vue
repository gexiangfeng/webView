  ...
  export default {
    ...
    created() {
	 ...
	 if(typeof sellerid !== 'undefined') {
	    var exp = new Date();
          exp.setTime(exp.getTime() + 3600 * 1000);//过期时间60分钟
          document.cookie = 'sellerid=' + sellerid + ";expires=" + exp.toGMTString();
      }
      //获取openid
      if(getCookie('openid') == null) {
    	  if(this.$route.query.sellerid == null){
    		  location.href = config.openidUrl + '?returnUrl=' +  encodeURIComponent(config.sellUrl + '/#/');
    	  }else{
    		  location.href = config.openidUrl + '?returnUrl=' +  encodeURIComponent(config.sellUrl + '/#/?sellerid=' + this.$route.query.sellerid);
    	  }
      ...
      this.$http.get(config.apiUrl+'/buyer/product/list', {
          params: {
